  ...
  const fixGiteeImageReferrer = () => {
    ...
    const processImages = () => {
      ...
      document.querySelectorAll('img').forEach(img => {
        ...
        // 如果是 Gitee 图片，进行处理
        if (src.includes('gitee.com') || lazySrc.includes('gitee.com')) {
          processImage(img)
        }
      })
    }
    ...
    document.addEventListener('lazyloaded', (e) => {
      const img = e.target
      if (img.tagName === 'IMG' && (img.src.includes('gitee.com') || img.getAttribute('data-lazy-src')?.includes('gitee.com'))) {
        processImage(img)
      }
    })
    
    // 使用 MutationObserver 监听图片 src 变化（懒加载时）
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && (mutation.attributeName === 'src' || mutation.attributeName === 'data-lazy-src')) {
    ...
    // 观察所有图片元素的属性变化
    document.querySelectorAll('img').forEach(img => {
      observer.observe(img, { attributes: true, attributeFilter: ['src', 'data-lazy-src', 'crossorigin'] })
    })
